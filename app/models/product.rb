class Product < ApplicationRecord
    ...
    time = Benchmark.realtime do
      ...
      ActiveRecord::Base.transaction do
        products_command =
          "copy products (price_list, brand, code, stock, cost, name, created_at, updated_at) from stdin with csv delimiter ';'"
  ...
  def self.import_action(input_hash, file_name)
    ...
    hsh['code'] = input_hash['Артикул'] || input_hash["﻿\"Артикул\""] || input_hash['Номер']

    if !hsh['brand'].empty? && !hsh['code'].empty? 
      hsh['price_list'] = file_name
      hsh['stock'] = input_hash['Количество'] || input_hash['Кол-во']
      hsh['stock'] = hsh['stock'].index('>').nil? ? hsh['stock'] : hsh['stock'].gsub('>', '')
      hsh['cost'] = input_hash['Цена'].tr(',', '.')
      hsh['name'] = input_hash['Наименование'] || input_hash['НаименованиеТовара']
      # puts("#{hsh['name']}")
      # TODO: How sanitize sql/csv here? Escape_symbol doesn't work. :(
      hsh['name'].gsub!('"', '') # грубое исправление проблемы
